             ...
             let rec loop()=
                actor{
                   ...
                   |Add (k,k_hash) ->    let value = toval k_hash
                                         ...
                                         let mutable row=0
                                         let mutable next="0"
                                        //  if nodeid="1036E3DDDC89A4E68D8A33F3823A180E" then 
                                        //      printfn "Leafmin:%A and lminv:%s" Leafsmin lminv
                                         if nodeid.Equals(k_hash) then
                                             printfn "in Node:%d for node:%d WTF" name k
                                             return()
                                        //  if Leafsmax.Count<8 || Leafsmin.Count<8 then 
                                        //       if value>nval && Leafsmax.Count<8 && not(Leafsmax.Contains(k_hash))then
                                        //            Leafsmax.Add(k_hash)
                                        //            Leafsmax.Sort()
                                        //            lmaxv<-Leafsmax.[Leafsmax.Count-1]
                                        //            lmax<-toval lmaxv
                                        //       if value<nval && Leafsmin.Count<8 && not(Leafsmin.Contains(k_hash))then
                                        //            Leafsmin.Add(k_hash)
                                        //            Leafsmin.Sort()
                                        //            lminv<-Leafsmin.[0]
                                        //            lmin<-toval lminv
                                             ...
                                             if value<=lmax && value>=lmin then
                                                ...
                                                if min<diff then
                                                 ...
                                                 if String.length(next)=1 then
                                                    ...
                                                    if min<diff then
                                                     next<-minleaf
                                                     //printfn "In All check, In node:%d with hash:%s and forwarding to:%s for new node:%s  " name n next k_hash
                  ...
                  // printfn "From Parent %A" msg
                   ...
                   |MDone ->   nodesreached<-nodesreached+1  
                              // if nodesreached%10=0 then
                               ...
                               if nodesreached=numNodes*numRequests then
                                 mflag<-false
...
let avg=totalhops/((float)numNodes*(float)numRequests)
